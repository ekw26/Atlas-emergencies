...
exclude_covid <- F # to exclude potential covid bias on prognosis set to T
td <- today()

if (sensitivity_analysis) {
  outputs_filepath <- glue("{results_filepath}{td}/sensitivity_analysis/")
  results_filepath <- glue("{results_filepath}{analysis_dt}/sensitivity_analysis")
} else {
  outputs_filepath <- glue("{results_filepath}{td}/")
  results_filepath <- glue("{results_filepath}{analysis_dt}")
}

if (offset) {
  outputs_filepath <- glue("{outputs_filepath}/offset/")
  results_filepath <- glue("{results_filepath}/offset")
}

if (exclude_covid) {
  outputs_filepath <- glue("{outputs_filepath}/exclude_covid/")
...
if (!offset) {
  ...
  if (!sensitivity_analysis) {
    p <- res %>%
      ...
      geom_line() + 
      geom_point(size = 1) + 
      geom_text(x = 1, y = 105, label = "Women", col = "black", size = 3) +  
      geom_text(x = 2, y = 105, label = "Men", col = "black", size = 3) +
      geom_text(data = . %>% filter(gender == "Women"), aes(x = 0.95, y = nudge_y, label = glue("{condition}, {signif(prop, 3)}%")), col = "grey30", hjust = "right", size = 2) + 
      geom_text(data = . %>% filter(gender == "Men"), aes(x = 2.05, y = nudge_y, label = glue("{signif(prop, 3)}%")), col = "grey30", hjust = "left", size = 2) +
      ylim(c(0, 105)) + 
      xlim(c(-0.3, 2.5)) +
      theme_classic() + 
      ...
      labs(col = "Statistically significant gender difference")
    
    ggsave(glue("{outputs_filepath}aurum_ED_freq.tiff"), p, width = 6, height = 4.16, units = "in", dpi = 300, compression = "lzw", create.dir = T)
    print(p)
    cat("  \n  \n\\newpage\n")
    ...
    p <- res %>%
      ...
      mutate(x = case_match(gender, 
             ...
             signif = case_when(is.na(ED.p) ~ 'Sex-specific condition', 
                                ED.p < 0.05 ~ "True", 
                                T ~ "False"), 
             nudge_y = case_when(pln == "gold" & gender == "Women" & condition == "Schizophrenia" ~ 43.05,
    ...
    ggsave(glue("{outputs_filepath}gold_ED_freq.tiff"), p, width = 6, height = 4.16, units = "in", dpi = 300, compression = "lzw", create.dir = T)
    print(p)
    cat("  \n  \n\\newpage\n")
  }
  ...
  plot_trends <- function(df, trend_var, pln1, gender1) {
    ...
    atlas_palette <- c(`Axial spondyloarthritis` = "blue3", `Brain tumours` = "#E31A1C", `Coeliac disease` = "green4", `CHD` = "#6A3D9A", `Colon cancer` = "#FF7F00", `COPD` = "orchid1", `IBD` = "black", `Lung cancer` = "#A65628", `Lyme disease` = "skyblue1", `Multiple sclerosis` = "blue3", `Ovarian cancer`= "#E31A1C", `Pancreatic cancer` = "green4", `Parkinson's disease` = "#6A3D9A", `PCOS` = "#FF7F00", `Rheumatoid arthritis` = "orchid1", `SBE` = "black", `Schizophrenia` = "#A65628", `Tuberculosis` = "skyblue1")
    ...
    p <- df %>%
      ...
      facet_wrap(grp ~ .) +
      geom_smooth(se = F, linewidth = 0.4) + geom_point(size = 0.7) +
      geom_errorbar(aes(ymin = low.ci, ymax = up.ci), width = 0.2, linewidth = 0.3, alpha = 0.5) +
      geom_label_repel(aes(label = label), nudge_x = nudge_x, na.rm = T, direction = "y", segment.linetype = 2, size = 1.75, fontface = "bold") +
  ...
  ggsave(glue("{outputs_filepath}diagyear_aurum_men.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
  print(p)
  cat("  \n  \n\\newpage\n")
  
  p <- plot_trends(timeres, 'diagyear', 'aurum', 'Women')
  ggsave(glue("{outputs_filepath}diagyear_aurum_women.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
  print(p)
  cat("  \n  \n\\newpage\n")
  
  p <- plot_trends(timeres, 'diagyear', 'gold', 'Men')
  ggsave(glue("{outputs_filepath}diagyear_gold_men.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
  print(p)
  cat("  \n  \n\\newpage\n")
  
  p <- plot_trends(timeres, 'diagyear', 'gold', 'Women')
  ggsave(glue("{outputs_filepath}diagyear_gold_women.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
  print(p)
  cat("  \n  \n\\newpage\n")
  ...
  ggsave(glue("{outputs_filepath}age_mid_aurum_men.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
  print(p)
  cat("  \n  \n\\newpage\n")
  
  p <- plot_trends(ageres, 'age_mid', 'aurum', 'Women')
  ggsave(glue("{outputs_filepath}age_mid_aurum_women.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
  print(p)
  cat("  \n  \n\\newpage\n")
  
  p <- plot_trends(ageres, 'age_mid', 'gold', 'Men')
  ggsave(glue("{outputs_filepath}age_mid_gold_men.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
  print(p)
  cat("  \n  \n\\newpage\n")
  
  p <- plot_trends(ageres, 'age_mid', 'gold', 'Women')
  ggsave(glue("{outputs_filepath}age_mid_gold_women.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
  print(p)
  cat("  \n  \n\\newpage\n")
  ...
  ggsave(glue("{outputs_filepath}imd_aurum_men.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
  print(p)
  cat("  \n  \n\\newpage\n")
  
  p <- plot_trends(imdres, 'imd', 'aurum', 'Women')
  ggsave(glue("{outputs_filepath}imd_aurum_women.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
  print(p)
  cat("  \n  \n\\newpage\n")
  
  p <- plot_trends(imdres, 'imd', 'gold', 'Men')
  ggsave(glue("{outputs_filepath}imd_gold_men.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
  print(p)
  cat("  \n  \n\\newpage\n")
  
  p <- plot_trends(imdres, 'imd', 'gold', 'Women')
  ggsave(glue("{outputs_filepath}imd_gold_women.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
  print(p)
  cat("  \n  \n\\newpage\n")
}
...
cat('# Prognosis - death')
cat('\n')
plot_deathORs <- function(group_pln = T, plnx = "Aurum") {
  df <- prognosis %>%
  ...
  if (group_pln) {
    ...
    p <- df %>%
      ...
      facet_wrap(gender ~ .) + 
      geom_point(aes(x = death, shape = pln), size = 1.25, position = position_dodgev(height = dodge_height))
  } else {
    dodge_height <- 0.6
    p <- df %>%
      filter(pln == plnx) %>%
      ggplot(aes(y = condition, colour = OR_type, group = OR_type)) + 
      facet_wrap(gender ~ .) + 
      geom_point(aes(x = death), size = 1.25, position = position_dodgev(height = dodge_height))
  }
  p <- p +
    ggplot2::geom_errorbarh(aes(xmin = death.low, xmax = death.up), height = .5, size = .4, position = position_dodgev(height = dodge_height)) + 
...
p <- plot_deathORs()
ggsave(glue("{outputs_filepath}death_ORs.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
print(p)

p <- plot_deathORs(group_pln = F)
ggsave(glue("{outputs_filepath}aurum_death_ORs.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
print(p)

p <- plot_deathORs(group_pln = F, plnx = "GOLD")
ggsave(glue("{outputs_filepath}gold_death_ORs.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
print(p)
cat("  \n  \n\\newpage\n")
cat('Aurum - prognosis')
...
plot_timeORs <- function(group_pln = T, plnx = "Aurum") {
  df <- prognosis %>%
  ...
  if (group_pln) {
    ...
    p <- df %>%
      ...
      facet_wrap(gender ~ .) + 
      geom_point(aes(x = time, shape = pln), size = 1.25, position = position_dodgev(height = dodge_height))
  } else {
    dodge_height <- 0.5
    p <- df %>%
      filter(pln == plnx) %>%
      ggplot(aes(colour = OR_type, y = condition, group = OR_type)) + 
      facet_wrap(gender ~ .) + 
      geom_point(aes(x = time), size = 1.25, position = position_dodgev(height = dodge_height))
  }
  p <- p +
    ggplot2::geom_errorbarh(aes(xmin = time.low, xmax = time.up), height = .5, size = .4, position = position_dodgev(height = dodge_height)) + 
    geom_vline(xintercept = 1, linetype = "dashed") + 
    coord_trans(x = "log10") + 
...
p <- plot_timeORs()
ggsave(glue("{outputs_filepath}time_ORs.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
print(p)

p <- plot_timeORs(group_pln = F)
ggsave(glue("{outputs_filepath}aurum_time_ORs.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
print(p)

p <- plot_timeORs(group_pln = F, plnx = "GOLD")
ggsave(glue("{outputs_filepath}gold_time_ORs.tiff"), p, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
print(p)
cat("  \n  \n\\newpage\n")
cat('Aurum - prognosis')
cat(knitr::knit_print(t3a))
...
if (!(offset | sensitivity_analysis)) {
  # illustrate excess poor outcomes in ED patients
  plot_excess <- function(plnx = "aurum", genderx = "Women") {
    
    df <- prognosis %>%
      left_join(res, by = join_by(condition, pln, gender))
    
    atlas_palette <- c(`Axial spondyloarthritis` = "blue3", `Coeliac disease` = "green4", `CHD` = "#E31A1C", `COPD` = "orchid1", `IBD` = "black", `Lyme disease` = "skyblue1", `Multiple sclerosis` = "#6A3D9A", `Parkinson's disease` = "#1B9E77", `PCOS` = "#FF7F00", `Rheumatoid arthritis` = "#A65628", `Schizophrenia` = "gold2", `Tuberculosis` = "#999999")
    ...
    a <- df %>%
      filter(n.EDdeath >= 10, n.NEDdeath >= 10, 
             !(condition %in% c('Brain tumours', 'Colon cancer', 'Lung cancer', 'Ovarian cancer', 'Pancreatic cancer', 'SBE')), 
             pln == plnx, 
             gender == genderx) %>%
      mutate(mort.diff = death.obs_true - death.obs_false) %>%
      ggplot(aes(death.obs_false, mort.diff, col = condition, size = prop)) + 
      geom_point() +
      theme_bw() + scale_colour_manual(values = atlas_palette, guide = "none") +
      scale_size_continuous(range = c(0.25, 3), limits = c(5, 55)) + 
      scale_y_continuous(limits = ylims) +
      geom_text_repel(aes(label = condition), na.rm = T, segment.linetype = 2, size = 1.75, fontface = "bold") +
      theme(text = element_text(size = 7, face = "bold")) + 
      labs(x = "One-year mortality (%) in non-emergency-diagnosed patients", 
           y = "Excess mortality (%) in emergency-diagnosed patients", 
           col = "", 
           size = str_wrap("% patients emergency-diagnosed", 10))
    ...
    b <- df %>%
      filter(n.EDadmitted >= 10, n.NEDadmitted >= 10, 
             ...
             pln == plnx, 
             gender == genderx) %>%
      mutate(hosp.diff = time.obs_true - time.obs_false) %>%
      ggplot(aes(time.obs_false, hosp.diff, col = condition, size = prop)) + 
      geom_point() +
      theme_bw() + scale_colour_manual(values = atlas_palette, guide = "none") +
      scale_size_continuous(range = c(0.25, 3), limits = c(5, 55)) + 
      ...
      geom_text_repel(aes(label = condition), na.rm = T, segment.linetype = 2, size = 1.75, fontface = "bold") +
      theme(text = element_text(size = 7, face = "bold")) +
      labs(x = "Mean nights in hospital in non-emergency-diagnosed patients", 
           y = "Excess mean nights in hospital in emergency-diagnosed patients", 
           col = "", 
           size = str_wrap("% patients emergency-diagnosed", 10))
  ...
  a <- plot_excess()
  ggsave(glue("{outputs_filepath}aurum_women_excess.tiff"), a, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
  
  
  b <- plot_excess(genderx = "Men")
  ggsave(glue("{outputs_filepath}aurum_men_excess.tiff"), b, width = 7.5, height = 4.16, units = "in", dpi = 300, compression = "lzw")
}
```
